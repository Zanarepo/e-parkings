...
export default function OperatorDashboard() {
  ...
  const loadData = async () => {
    ...
    const spaces = await ParkingSpace.filter({ operator_id: currentUser.id });
  ...
  // Occupancy rate calculation variables are kept but the card displaying them is removed from the top grid as per outline.
  const totalSpots = parkingSpaces.reduce((sum, s) => sum + s.total_spaces, 0);
  const occupiedSpots = totalSpots - parkingSpaces.reduce((sum, s) => sum + (s.available_spaces || s.total_spaces), 0);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">Welcome back, {user?.business_name || user?.full_name}</p>
          </div>
          <Button
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card className="shadow-lg hover:shadow-xl transition-shadow">
            <CardContent className="p-6">
              ...
              <p className="text-3xl font-bold text-emerald-600 mt-1">₦{operatorEarnings.toFixed(2)}</p>
              <p className="text-xs text-gray-500 mt-2">85% of ₦{totalRevenue.toFixed(2)} total revenue</p>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-lg hover:shadow-xl transition-shadow">
            <CardContent className="p-6">
              ...
              <p className="text-xs text-gray-500 mt-2">From ₦{todayRevenue.toFixed(2)} total revenue</p>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-lg hover:shadow-xl transition-shadow">
            <CardContent className="p-6">
              ...
              <p className="text-2xl font-bold text-gray-900 mt-1">₦{platformCommission.toFixed(2)}</p>
        ...
        <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg mb-8">
          ...
          <p className="text-sm text-blue-800">
            ...
            <br />
            • Platform takes <strong>15%</strong> commission for payment processing, QR system, and maintenance
            <br />
            • Commission is automatically calculated on checkout
          </p>
        </div>
              ...
              {parkingSpaces.length === 0 ? (
                ...
                <div className="space-y-3">
                    ...
                    >
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {sessions.filter(s => s.parking_space_id === space.id && s.status === "active").length > 0 && (
                            <Badge className="bg-blue-100 text-blue-700">
                              {sessions.filter(s => s.parking_space_id === space.id && s.status === "active").length} active
                            </Badge>
                          )}
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-semibold text-emerald-600">
                          {space.available_spaces || space.total_spaces}/{space.total_spaces}
                        </p>
                  ...
                  {parkingSpaces.length > 5 && (
                    ...
                    >
                      View All {parkingSpaces.length} Spaces
                    </Button>
                  )}
                </div>
              )}
                  ...
                  {sessions.slice(0, 5).map(session => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{session.parking_space_name}</p>
