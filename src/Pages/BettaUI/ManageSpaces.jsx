...
export default function ManageSpace() {
  ...
  const loadData = async () => {
    const spaces = await ParkingSpace.filter({ id: spaceId });
    if (spaces.length > 0) {
      setParkingSpace(spaces[0]);
    }

    const allSessions = await ParkingSession.filter(
  ...
  const toggleSpaceStatus = async () => {
    const newStatus = parkingSpace.status === "active" ? "inactive" : "active";
    await ParkingSpace.update(parkingSpace.id, { status: newStatus });
    await loadData();
  };

  const copyQRCode = () => {
    if (!parkingSpace || !parkingSpace.qr_code) {
      console.warn("No QR code to copy.");
      return;
    }
  ...
  const getTodayRevenue = () => {
    ...
    return sessions
      .filter(s => {
        if (!s.check_out_time || s.status !== "completed") return false;
  ...
  const getOccupancyRate = () => {
    ...
    const occupied = parkingSpace.total_spaces - (parkingSpace.available_spaces || parkingSpace.total_spaces);
    return parkingSpace.total_spaces > 0 ? ((occupied / parkingSpace.total_spaces) * 100).toFixed(1) : (0).toFixed(1);
  };
  ...
  return (
    <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <Button
          variant="outline"
          onClick={() => navigate(createPageUrl("MySpaces"))}
          className="mb-6"
        >
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">{parkingSpace.name}</h1>
          ...
          <div className="flex gap-2">
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="flex flex-col items-center py-6">
                  <div className="w-full p-8 bg-gradient-to-br from-emerald-50 to-yellow-50 rounded-2xl border-4 border-emerald-500 shadow-xl">
                    <p className="text-sm text-gray-600 text-center mb-4 font-semibold">PARKING CODE</p>
            ...
            <Button
              ...
              className={parkingSpace.status === "active" ? "border-red-300" : "border-emerald-300"}
            >
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          <Card className="shadow-lg">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <Badge className={parkingSpace.status === "active" ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-700"}>
                  {parkingSpace.status}
                </Badge>
              </div>
              <p className="text-sm text-gray-600">Occupancy Rate</p>
              <p className="text-3xl font-bold text-gray-900">{getOccupancyRate()}%</p>
              <p className="text-xs text-gray-500 mt-1">
                {parkingSpace.available_spaces || parkingSpace.total_spaces}/{parkingSpace.total_spaces} available
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-lg">
            <CardContent className="p-6">
              ...
              <p className="text-2xl font-bold text-gray-900">₦{getTodayRevenue().toFixed(2)}</p>
        ...
        <Tabs defaultValue="active" className="w-full">
          <TabsList className="grid w-full grid-cols-4 mb-6">
            <TabsTrigger value="active">Active Now ({activeSessions.length})</TabsTrigger>
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                ...
                                <div>
                                  <h4 className="font-semibold">{session.driver_name}</h4>
                              ...
                              <div className="grid grid-cols-2 gap-2 text-sm">
                                <div>
                                  ...
                                  <p className="font-medium">{format(new Date(session.check_in_time), "h:mm a")}</p>
                                </div>
                                <div>
                                  <p className="text-gray-600">Hourly Rate</p>
                                  <p className="font-medium">₦{session.hourly_rate}/hr</p>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex-1">
                              ...
                              <div className="grid grid-cols-2 gap-2 text-sm">
                                ...
                                <div>
                                  ...
                                  <p className="font-medium">{session.pause_time ? format(new Date(session.pause_time), "h:mm a") : 'N/A'}</p>
                                </div>
                              </div>
                            </div>
                  ...
                  {sessions.filter(s => s.status === "completed").map(session => (
                    <Card key={session.id} className="border-gray-200">
                      <CardContent className="p-4">
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <h4 className="font-semibold mb-1">{session.driver_name}</h4>
                            <p className="text-sm text-gray-600 mb-2">{session.vehicle_plate}</p>
                            <div className="grid grid-cols-3 gap-2 text-sm">
                              <div>
                                <p className="text-gray-600">Duration</p>
                                <p className="font-medium">{session.total_hours}h</p>
                              </div>
                              <div>
                                <p className="text-gray-600">Total Paid</p>
                                <p className="font-medium">₦{session.total_amount?.toFixed(2)}</p>
                              </div>
                              <div>
                                <p className="text-gray-600">You Earned</p>
                                <p className="font-medium text-emerald-600">₦{session.operator_earnings?.toFixed(2)}</p>
                              </div>
                            </div>
                          </div>
                          <div className="text-right text-xs text-gray-500">
                            {format(new Date(session.check_out_time), "MMM d, h:mm a")}
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                        ...
                        <div className="flex justify-between items-start">
                          ...
                          {session.operator_earnings && (
                            <div className="text-right">
                              <p className="font-semibold text-emerald-600">₦{session.operator_earnings.toFixed(2)}</p>
                              <p className="text-xs text-gray-500">from ₦{session.total_amount.toFixed(2)}</p>
                            </div>
                          )}
                        </div>
                      </CardContent>
